...
main() {
	...
	// TODO calcular consumo de memï¿½ria.
	...
	while (no != NULL) {
		...
		for (i = 0; i < strlen(conteudoLinha); i++) {
			...
			if ((aspas == 2) && ((valorAscii == 32) || (valorAscii == 36) || (valorAscii == 44) || (valorAscii == 59))) {
				...
				// como $ faz parte da variavel e tambem ï¿½ uma condicao de parada, entao preciso incrementar aqui
				if (valorAscii == 36) {
					palavraAux[count] = (char) valorAscii;
					count++;
				}
...
int validarInstrucaoLeia(char * palavra, int nuLinha, char * linha) {
	...
	// verifica se a palavra reservada ï¿½ 'ler'
	if (strcmp(palavra, palavrasReservadas[0]) == 0) {

		// identifica se a palavra reservada ï¿½ 'ler'
		isValido = 1;
...
int validarInstrucaoEscreva(char * palavra, int nuLinha, char * linha) {
	...
	if (strcmp(palavra, palavrasReservadas[8]) == 0) {
		...
		// identifica se a palavra reservada ï¿½ 'escrever'
		isValido = 1;
...
void getTamanhoVariavel(char palavra[], char retorno[], int numeroLinha) {
	...
	for (i = tamanhoPalavra; i >= 0; i--) {
		...
		// verifica se o ultimo caracter ï¿½ um ], caso seja vai percorrendo para salvar o tamanho
		if (i == tamanhoPalavra && valorAscii == 93) {
			hasPossuiConchete++;
			continue;
			// condiï¿½ï¿½o que ira acumular enquanto for numero e nao encontrar
...
int validaPalavraString(char* palavra, int numeroLinha) {
	...
	// verifica se ï¿½ impar, caso seja o duplo balanceamento esta invalido.
...
/**
 * Verifica se a gramatica possui uma funcao principal vï¿½lida.
 *
 * @param char palavra
 * @param int nuLinha
 */
...
int insereInicioTabelaSimbolo(TabelaSimbolo* lista, Simbolo simbolo) {
    ...
    //lista nï¿½o vazia: apontar para o anterior!
    if (*lista != NULL) {
        (*lista)->ant = no;
    }
